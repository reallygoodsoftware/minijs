...
const MiniJS = (() => {
  ...
  const watchHandler = {
    set: function(target, property, value) {
      ...
      if (typeof value === 'object' && value !== null) {
        proxyValue = new Proxy(value, watchHandler);
        proxyValue.__parent__ = variable;
      }
  ...
  function _setProxyVariables() {
    _variables.forEach((variable) => {
      if (['proxyWindow', 'MiniJS'].includes(variable)) return;
      
      const value = proxyWindow[variable];
      if (typeof value === 'object' && value != null && !value.__proxy__) {
        proxyWindow[variable] = new Proxy(value, watchHandler);
        proxyWindow[variable].__parent__ = variable;
      }
    });
  }
