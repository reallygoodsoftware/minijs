...
const MiniJS = (() => {
  ...
  function _setUserDefinedVariables() {
    ...
    _variables = currentProps.filter(p => !nativeProps.includes(p) && !_ignoredVariables.includes(p));
  }
  ...
  function updateStates(property = null) {
    _elements.forEach(entity => {
      if (entity.variables.includes(property) || property == null || entity.uuid == property || entity.parent?.uuid == property || entity.dependencies.includes(property))
      {
        entity.evaluateEach()
        entity.evaluateAll()
      }
