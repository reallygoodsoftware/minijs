...
const MiniJS = (() => {
  ...
  function _addArrayMethodsToVariable(variable)
  {
    ...
    function toggle(value) {
      ...
      if (Array.isArray(value)) {
        ...
        let index = variable.indexOf(value);
        if (index === -1) {
          newValue = variable.concat(value);
        } else {
          newValue = variable.slice(0, index).concat(variable.slice(index + 1));
        }
      }
      variable = newValue;
    };
    ...
    function subtract(arr) {
      ...
      const result = variable.filter(x => !arr.includes(x));
      return _addArrayMethodsToVariable(result);
    }
    ...
    function search(input) {
      ...
      const matches = variable.filter(tag => {
          const lowerTag = tag.toLowerCase();
          return inputWords.every(word => lowerTag.includes(word));
      });
  
      return _addArrayMethodsToVariable(matches);
    }
  ...
  function _addMethodsToVariables(variables = _variables) {
    variables.forEach((variable) => {
      _addArrayMethodsToVariable(proxyWindow[variable])
    })
  }
