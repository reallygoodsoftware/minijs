  ...
  function _addMethodsToVariables() {
    _variables.forEach((variable) => {
      if( Array.isArray(proxyWindow[variable]))
      {
        ...
        function searchClosest(target, arr) {
          const lowerTarget = target.toLowerCase();
        
          // Filter the array to get only the strings that include the target, ignoring case
          const matches = arr.filter(item => item.toLowerCase().includes(lowerTarget));
        
          // Sort the matches based on the absolute difference in length
          // between the target and each match
          matches.sort((a, b) => {
            return Math.abs(lowerTarget.length - a.length) - Math.abs(lowerTarget.length - b.length);
          });
        ...
        function searching(value) {
          const result = searchClosest(value, this)
          return result
        }
