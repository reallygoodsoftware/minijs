...
export default class Entity {
  ...
  _sanitizeContentExpression(expr, options = {}) {
    if (options.isClass || expr.includes("el.")) {
      const lexer = new Lexer(expr, options);
  ...
  evaluateClass() {
    ...
    const output = this._sanitizeContentExpression(expr, { isClass: true });
    let newClassNames = [...this.baseClasses];

    if (typeof output === 'string')
      newClassNames.concat(output.split(' '));
    
    if (Array.isArray(output)) {
      output.forEach((conditional) => {
        const condition = eval(conditional.test);
        const consequent = eval(conditional.consequent).split(' ');
        const alternate = eval(conditional.alternate).split(' ');
