...
export default class Entity {
  ...
  getVariables() {
    ...
    const filtered = [...new Set(allVariables.filter(v => attributeToken.includes(v)))]


    for (let token of filtered) {
      if (typeof window[token] === 'function') {
        filtered.push(allVariables.find(v => window[token].toString().includes(v)))
      }
