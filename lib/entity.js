import { Interpreter, ClassInterpreter } from './interpreter';
...
export default class Entity {
  ...
  evaluateClass() {
    ...
    this.element.className = this._interpret(expr, {
  ...
  evaluateEach() {
    ...
    if (eachExpr) {
      ...
      const items = this._interpret(iterable);
  ...
  evaluateDynamicAttributes() {
    this.dynamicAttributes.forEach((attr) => {
      ...
      const newValue = this._interpret(expr)
  ...
  evaluateText() {
    ...
    const newText = this._interpret(textExpr)
  ...
  evaluateValue() {
    ...
    if (valueExpr) {
      const newValue = this._interpret(valueExpr)
    ...
    if (checkedExpr) {
      const newValue = this._interpret(checkedExpr)
