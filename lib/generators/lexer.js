...
- add support for object destructuring
*/
      ...
      VariableDeclarator(node) {
        const names = (
          node.id.type === 'ArrayPattern'
            ? node.id.elements.map((element) => element.name)
      ...
      AssignmentExpression(node) {
        const names = (
          node.left.type === 'ArrayPattern'
            ? node.left.elements.map((element) => element.name)
        ...
        AssignmentExpression(node) {
          ...
          names.forEach((name) => {
            ...
            const idNode =
              node.left.type === 'Identifier'
                ? node.left
                : node.left.elements.find((element) => element.name === name)
