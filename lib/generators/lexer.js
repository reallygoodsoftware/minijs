...
export class Lexer {
  ...
  get identifiers() {
    ...
    this._identifiers = [...new Set(this._identifiers)].filter(
      (id) => !isNativeVariable(id)
    )
    this._declaredIdentifiers = [...new Set(this._declaredIdentifiers)]

    return this._identifiers.filter((id) => !Lexer.IGNORED_IDS.includes(id))
  }
