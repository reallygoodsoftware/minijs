...
export class Lexer {
  ...
  isDeclared(id, state) {
    ...
    while (ancestors.length) {
      ...
      const declarations = getDeclaredVariables(currentNode)
  ...
  get identifiers() {
    ...
    walk.fullAncestor(this._ast, (node, state, parent) => {
      if (node?.type === 'MemberExpression') {
        ...
        if (Lexer.ENTITY_KEYS.includes(object)) {
        ...
        const declarations = getDeclaredVariables(node)
  ...
  output() {
    ...
    if (Lexer.debug)
      console.log({ type: 'Lexer.output', input: this._code, output })
  ...
  conditional() {
    ...
    if (Lexer.debug)
      console.log({ type: 'Lexer.conditional', input: this._code, output })
