...
export class State {
  ...
  removeEntity(entity) {
    ...
    const dependencies = [...this.dependencies.entries()]
    dependencies.forEach(([key, value]) => {
      if (key === entity.id) this.dependencies.delete(key)
    ...
    const entityDependencies = [...this.entityDependencies.entries()]
    entityDependencies.forEach(([key, value]) => {
      const [entityID] = key.split('.')
      if (entityID === entity.id) this.entityDependencies.delete(key)
      else if (value.includes(entity.id))
        this.entityDependencies.set(
          key,
          value.filter((id) => id !== entity.id)
        )
    })

    MiniJS.elements = MiniJS.elements.filter((el) => el.id !== entity.id)
  }
  ...
  attachVariableHelpers(variables) {
  ...
  disposeVariables(entityID, variables) {
    variables.forEach((variable) => {
      if (State.isElState(variable)) {
        ...
        MiniJS.state.disposeDependency(entityID)

        if (variable !== 'el') {
          const varName = variable.replace('el.', '')
          MiniJS.state.disposeEntityDependency(entityID, varName)
        }
      } else {
